    ...
    function loop(now: number) {
      ...
      for (const p of pkts) {
        ...
        if (!p.isResponse && NODE_TYPE_MAP.get(arrived) === 'ТСПУ') {
          ...
          if (p.kind === 'tunnel') { tspuStatsRef.current.bypass++ }  // VPN bypasses any mode
            ...
            if (blockedByMode) {
              ...
              spawnShards(arrived); tspuFlashRef.current = now; SFX.PACKET_BLOCKED()
        ...
        if (p.seg >= p.nodes.length - 1) {
          ...
          if (p.kind === 'http')   { cDel++; stat.passed++; useStore.getState().scorePacket('http'); spawnIncome(arrived, PKT_INCOME.http); spawnResponse(p); SFX.PACKET_DELIVERED()
            setHudEvents(prev => [{ ts: nowTs(), icon: '✓' as const, text: `${srcLbl} → ${dstSub} | TCP | ${lat}мс` }, ...prev].slice(0, 8)) }
          if (p.kind === 'tunnel') { cVpn++; cDel++; stat.passed++; useStore.getState().scorePacket('tunnel'); spawnIncome(arrived, PKT_INCOME.tunnel); spawnResponse(p); SFX.VPN_TUNNELED()
            setHudEvents(prev => [{ ts: nowTs(), icon: '⚡' as const, text: `${srcLbl} → blocked.com | VPN туннель | ${lat}мс` }, ...prev].slice(0, 8)) }
          if (p.kind === 'dns')    { cDns++; stat.passed++; useStore.getState().scorePacket('dns'); spawnIncome(arrived, PKT_INCOME.dns); SFX.DNS_RESOLVED()
